...
from models import GAT, EmbedGAT
...
biases = process.adj_to_bias(adj, [nb_nodes], nhood=args.nhood)
...
with tf.Graph().as_default():
    ...
    logits = model.inference(ftr_in, nb_classes, nb_nodes, is_train,
                                ...
                                residual=residual, activation=nonlinearity, export_dict=export_dict)
    ...
    with tf.Session(config=tf_config) as sess:
            ...
            while tr_step * batch_size < tr_size:
                _, loss_value_tr, acc_tr = sess.run([train_op, loss, accuracy],
                    feed_dict={
                        ...
                        is_train: True,
                        attn_drop: args.attn_drop, ffd_drop: args.ffd_drop})
        ...
        ts_size = features.shape[0]
        ts_step = 0
        for scope, tensor_dict in export_dict.items():
            for tensor_name, tensor in tensor_dict.items():
                ...
                # if args.use_signac:
                #     print(f"Saving tensor {scope}/{tensor_name} to job data storage...")
                #     job.data[f"{scope}/{tensor_name}"] = tensor_value
